...
# Configuration
LOGTO_ADMIN_URL="https://localhost:3302"
LOGTO_STATUS_URL="https://localhost:3301/api/status"
APP_NAME="breakdown dev"
ENV_FILE=".env.idp"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"

# Trust the dev CA for curl (generated by scripts/generate-dev-certs.sh).
# Falls back to --insecure if the CA has not been generated yet.
if [[ -f "$REPO_ROOT/dev-certs/ca.pem" ]]; then
  CURL_CA_ARGS=("--cacert" "$REPO_ROOT/dev-certs/ca.pem")
else
...
retries=0
until curl -fsSL "${CURL_CA_ARGS[@]}" "$LOGTO_STATUS_URL" > /dev/null 2>&1; do
    retries=$((retries + 1))
...
echo "   On first boot, Logto requires admin user setup via the UI at https://localhost:3302"
...
echo "   1. Open https://localhost:3302 in your browser"
...
# Dev-mode values for local Logto (HTTPS with dev-pinned CA):
# - Issuer: https://localhost:3301
# - JWKS: https://localhost:3301/.well-known/jwks
...
#
# IMPORTANT: After initial Logto setup via UI (https://localhost:3302),
# update these values with your actual Application ID / Resource Indicator.

# Issuer URL (points to local Logto OIDC endpoint, HTTPS with dev CA)
OIDC_ISS=https://localhost:3301
...
OIDC_JWKS_URL=https://localhost:3301/.well-known/jwks
EOF
...
echo "   2. Complete admin setup: https://localhost:3302"
