...
Rotation is not an in-place Transit version bump: changing the bucket-level DEK makes existing objects unreadable. The runbook therefore requires a maintenance/backfill job that preserves old ciphertext in a durable rollback copy with a manifest, writes and verifies candidate objects under a staging prefix, and then promotes the candidate by writing it to the same `kv/data/photo-sse-c` path with `options.cas=<expected-active-version>`. A CAS conflict leaves the winner active and requires reconciliation; a failed backfill restores canonical objects from the rollback copy and retains staging and rollback artifacts until the outcome is known. During migration and before any key destruction, all API/photo workers must be stopped or quiesced, with explicit verification that every OpenDAL operator is released. The runbook records that destroying `photo-sse-c` before backfill and operator shutdown is a deliberate whole-bucket crypto-shred operation.
