...
- [ ] 5.4 Implement `ScriptImportWorker`: dequeue → pdftotext → chunk → per-chunk `LlmClient` (bounded by `max_chunks_per_script`) → assemble `ScriptContext` preview blob → store + project preview + telemetry
- [ ] 5.5 Implement `ScheduleImportWorker`: dequeue → CSV parse or LLM → `ShootingSchedule` preview blob → telemetry
- [ ] 5.6 Implement `MergeWorker`: dequeue → load schedule preview + Scene read-model projection by block → `merge_schedule_to_scenes` → block until block has applied scenes → store merged preview + unmatched lists
...
- [ ] 6.6 Register all routes on the production router + `ApiDoc` (utoipa) OpenAPI surface
