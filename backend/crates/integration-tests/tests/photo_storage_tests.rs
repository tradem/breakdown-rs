...
fn test_image_bytes() -> Vec<u8> {
    ...
    vec![
        ...
        0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x11, 0x05, 0x12, 0x21, 0x31, 0x41,
        ...
        0x01, 0x00, 0x00, 0x3f, 0x00, 0x7b, 0x94, 0x11, 0x00, 0x00, 0x00, 0x00,
...
async fn photo_storage_store_fetch_delete_round_trip() -> Result<()> {
    ...
    storage
        .store(photo_id, PhotoVariant::Original, bytes.clone(), content_type.clone())
        .await?;

    // Fetch it back and verify.
    let fetched = storage
        .fetch(photo_id, PhotoVariant::Original)
        .await?;
...
async fn photo_storage_store_multiple_variants() -> Result<()> {
    ...
    storage
        .store(photo_id, PhotoVariant::Original, original.clone(), "image/jpeg".into())
        .await?;
    storage
        .store(photo_id, PhotoVariant::Thumb, thumb.clone(), "image/jpeg".into())
        .await?;
    storage
        .store(photo_id, PhotoVariant::Medium, medium.clone(), "image/jpeg".into())
        .await?;
...
async fn photo_storage_delete_all_removes_all_variants() -> Result<()> {
    ...
    // Store two variants.
    storage
        .store(photo_id, PhotoVariant::Original, bytes.clone(), "image/jpeg".into())
        .await?;
    storage
        .store(photo_id, PhotoVariant::Thumb, bytes.clone(), "image/jpeg".into())
        .await?;
...
async fn photo_storage_overwrite_by_default() -> Result<()> {
    ...
    storage
        .store(photo_id, PhotoVariant::Original, updated.clone(), "image/jpeg".into())
        .await?;
...
async fn photo_storage_list_returns_known_ids() -> Result<()> {
    ...
    storage
        .store(id_a, PhotoVariant::Original, bytes.clone(), "image/jpeg".into())
        .await?;
    storage
        .store(id_b, PhotoVariant::Original, bytes.clone(), "image/jpeg".into())
        .await?;
    ...
    let ids = storage.list().await?;
    assert!(!ids.contains(&id_a), "list should NOT contain id_a after delete");
