...
> Long-form rationale, examples and reference material live as on-demand instructions in the
> **monorepo root** `.github/instructions/`, injected when matching files are read
> (via `pi-rules` / GitHub-Copilot-instructions — see the §6 register). Note: pi-rules discovers the
> instructions only when pi is launched from the **repo root** (`~/…/breakdown-rs`), so prefer
> root-launch sessions for full rule coverage. Section numbers (§1–§5, §7) and titles
...
Detailed documentation lives in the **monorepo root** `.github/instructions/` (Markdown with YAML
frontmatter; loaded via `pi-rules`/Copilot-instructions when matching files are read — glob rules are
**dynamic**: they inject on matching file reads and do not appear in `/rules`, which lists only
always-on rules). Rule bodies are capped at 12k chars per rule. Keep the split discipline when
moving or adding rules: **normative stays in this file, descriptive goes to the instruction**
that scopes it. Globs are repo-root-relative.

| File (under `.github/instructions/`) | loaded for | Content |
|---|---|---|
| `architecture-hard-rules.instructions.md` | `backend/crates/**/*.rs` | Hard-rules long form (CQRS boundary, no panics, SQL, AUTHZ-GATE, ADR-031, reliability) |
| `domain-model.instructions.md` | `backend/crates/{core,infra,api}/src/**` | Domain model: hierarchy, aggregates, invariants, seeding saga |
| `photo-context.instructions.md` | `backend/crates/*/src/photo/**`, backend compose/scripts | Photo bounded context, sagas, Garage/S3 env, GC |
| `ai-import.instructions.md` | `backend/crates/*/src/ai/**`, `backend/config/default_ai_prompts.toml` | AI import: env, concurrency permits, payload storage/GC, restart recovery |
| `integration-tests.instructions.md` | `backend/crates/integration-tests/**` | Tiers, local execution, troubleshooting, gotchas, CI prerequisites |
| `local-dev-runtime.instructions.md` | backend compose/scripts/`.env*`/dev-certs, root `scripts/**` | Dev runtime, boot sequence, env vars, OIDC/dev-auth, IdP overlay |
| `ci-hardening.instructions.md` | `.github/workflows/**`, `backend/rules/**` | Guardrail job detail, SHA-pinning, script-injection hygiene |
