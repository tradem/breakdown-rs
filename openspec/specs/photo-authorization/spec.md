...
Provides season-scoped authorization for costume-photo access (upload,
download, unlink). V1 derives access from active costume-department block
membership in the target season. A v2 evolution to an additive `SeasonCrew`
aggregate is documented but not implemented in v1.
