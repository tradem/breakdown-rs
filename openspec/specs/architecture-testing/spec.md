...
Automated enforcement of hexagonal architecture boundaries to prevent erosion of the core domain boundary. Two-layer strategy: `cargo-deny` for dependency-level bans and `rust_arkitect` for source-level `use` statement checks. See ADR-017.
...
- **THEN** the dependency check runs and report pass/fail within 10 seconds
