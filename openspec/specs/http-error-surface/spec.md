...
Provides a unified RFC 9457 `application/problem+json` error contract for every HTTP failure the API can produce: domain errors, auth middleware rejections, extractor/rejection failures, unknown routes, payload-limit rejections, and the panic fallback. Stable `{context}.{reason}` codes in a single registry are the machine contract for clients; per-code extension whitelists with an S0/S1/S2 privacy classification govern what identifiers may leave the server; `trace_id` correlates user-visible failures with otel telemetry.
