...
# flutter-scene-shoots-screen Specification

## Purpose
Defines the shoot-day execution surface of the Flutter client: the day
board for a shooting day rendering its scene shoots in sequence with the
Ist state (started/finished/skipped, actual order, day finality from
`wrapped_at`), the execution commands (plan/replan/start/actual-order/
finish/skip/notes/wrap) dispatched with version echoes and the
optimistic-after-2xx + bounded-retry reconciliation discipline, the
guarded wrap with finality copy, and the continuity photos bound to
scene shoots (AUTHZ-GATED capture/list/unlink reusing the Phase 2
capture pipeline).
...
The day board SHALL render the day's scene shoots in
...
Continuity capture SHALL reuse the Phase 2 capture pipeline (point-of-use permission rationale, isolate prepare,
