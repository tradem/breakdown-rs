...
## 1. Controller
- [x] 1.1 `features/seasons/seasons_controller.dart` — `@riverpod`
       `SeasonsController` returning `AsyncValue<List<SeasonDto>>`
- [x] 1.2 `create(name)` returns `Result<SeasonDto, ProblemError>`;
...
## 3. Screen
- [x] 3.1 `SeasonsScreen` `ConsumerWidget` (no StatefulWidget/setState)
- [x] 3.2 AppBar, ListView, FAB → bottom-sheet Create Season form
- [x] 3.3 AUTHZ-GATE on the FAB (hide if no create role)

## 4. Optimistic + lag reconciliation
- [x] 4.1 Optimistic insert **after** `POST /v1/seasons` 2xx, as a
       controller-state overlay (NOT a Drift write) keyed by the server `id`
       (Design Decision D1/D2)
- [x] 4.2 Bounded-retry refetch; on timeout retain the overlay with
       `stale = true` (overlay only, Drift untouched) + pull-to-refresh
       (Design Decision D2/D3)
- [x] 4.3 Pull-to-refresh

## 5. Tests
- [x] 5.1 Unit: mapper, repository Ok + Err branches
- [x] 5.2 Widget test + golden for `SeasonsScreen`
- [x] 5.3 integration_test smoke against a mocked API
- [x] 5.4 Unit + widget: POST network/5xx failure → no overlay, Drift
       untouched, `AsyncError` keyed on `code` (Design Decision D3)
- [x] 5.5 Unit + widget: `409` conflict (command-side, before 2xx) → no overlay
       created (nothing to revert), `AsyncError` keyed on `code` (spec scenario)
- [x] 5.6 Unit + widget: bounded-retry exhaustion → overlay retained
       `stale`, non-fatal warning + pull-to-refresh, Drift untouched
       (Design Decision D3)

## 6. Pattern documentation
- [x] 6.1 Note in `AGENTS.md` §9 that this screen is the reference pattern
