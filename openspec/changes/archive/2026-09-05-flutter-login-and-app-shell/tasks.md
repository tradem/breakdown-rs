...
- [x] 1.1 `lib/design/theme.dart` — `AppThemes.light()` / `AppThemes.dark()`
       (`ColorScheme.fromSeed`, teal seed token) and
       `lib/design/spacing.dart` token set
- [x] 1.2 `lib/app.dart` — `App` renders Consumer with `theme`,
...
## 2. Auth gate
- [x] 2.1 `lib/app.dart` — root ConsumerWidget switch over
...
- [x] 2.2 Widget tests: all four gate states; assert no main-app network
...
- [x] 3.1 `features/auth/login_screen.dart` — `ConsumerWidget`; pure
...
- [x] 3.2 `lib/auth/platform_authorization_ui.dart` — `AuthorizationUi`
...
- [x] 3.4 Add `url_launcher` + `app_links` to `pubspec.yaml`
- [x] 3.5 Unit tests: fake `AuthorizationUi` driving `signIn()` Ok/Err
