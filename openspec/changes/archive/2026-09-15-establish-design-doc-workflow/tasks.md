...
- [x] 1.1 Create `docs/design/screens/README.md`: template with the
...
- [x] 2.1 Create `docs/design/glossary.md` with the table columns
      ...
      "Season erstellen")
- [x] 2.2 Formulate the visible-label rule as a norm: every
      navigation destination and primary action gets a visible text
      label; icons reinforce, they never carry meaning alone
- [x] 2.3 Audit the icons in use under `lib/features/**` against the
      glossary and fill the gaps (documentation only — no code
      changes)

## 3. Pi Skills

- [x] 3.1 Create `frontend-flutter/.pi/skills/design-wireframe-salt/
...
- [x] 3.2 Extend the skill with a sample Salt block (from report
      §4.2 target wireframe) and the static-only rule as acceptance
      criteria
- [x] 3.3 Extend `openspec-screen-prompt/SKILL.md`: (a) reference
...
- [x] 4.1 Small script `scripts/check-design-diagrams.sh`
...
- [x] 4.2 Provide PlantUML jar in a versioned way (download URL +
...
- [x] 4.3 GitHub workflow `docs-design-lint.yml` (or extension of an
...
- [x] 4.4 Run the script locally against the research report and the
      template (prove all blocks compile) — the report's blocks
      serve as the test fixture

## 5. Documentation & Verification

- [x] 5.1 Short note in `frontend-flutter/AGENTS.md` and the
...
- [x] 5.2 Run `openspec validate` for this change and check all
      artifacts (proposal ↔ specs ↔ tasks consistency)
- [x] 5.3 Trial run: an agent authors a mini screen spec from the
