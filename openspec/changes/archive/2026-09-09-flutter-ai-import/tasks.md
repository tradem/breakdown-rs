...
## 1. Data layer
- [x] 1.1 `data/ai_config_repository.dart` — providers/models lists,
...
- [x] 1.2 `data/ai_import_repository.dart` — raw-body `uploadSchedule`
...
- [x] 1.3 Secure-storage `ai_config_id` + recent-`job_ids` hand-off
...
- [x] 1.4 Drift `ai_import_jobs` table + migration; repository
       cache-write discipline (success only, snapshot rules)
- [x] 1.5 Unit tests: every route Ok/Err (incl. 200/202/413/415/404/
...
- [x] 2.1 `features/ai_import/ai_config/` — first-run → provider →
...
- [x] 2.2 Widget tests + goldens (light/dark × android/macos):
       first-run/edit/denied/conflict states; masked-key field
       semantics

## 3. Submission + status feature
- [x] 3.1 `features/ai_import/import_jobs/import_submit_screen.dart`
...
- [x] 3.2 `job_status_screen.dart` + `jobRepository.watch(jobId)`
...
- [x] 3.3 Unit tests: watch state machine (fake scheduler; terminal
       + unsubscribe stop; no wall-clock gating)
- [x] 3.4 Widget tests + goldens: all six statuses, duplicate
       callout, 413/415/403/404 copy

## 4. Preview + apply feature
- [x] 4.1 `preview_screen.dart` — typed `AiImportPreviewResponse` /
       `AiPreviewPayload` (`kind`/`data`: script/schedule/merged,
       backend issue #337) rendering; degraded unknown-`kind` cards,
       404-empty state
- [x] 4.2 `apply_controller.dart` — mapping request builder (Create /
...
- [x] 4.3 Unit tests: typed-preview rendering (script/schedule/merged
...
- [x] 4.4 Widget tests + goldens: preview cards, degraded rows,
       apply summary, 409/403 branches

## 5. AUTHZ-GATE seam
- [x] 5.1 Uploads pre-gated via the Phase 2 capability gate
       (`// AUTHZ-GATE:` annotated); config screens session-only
       (documented exception, no capability surface)
- [x] 5.2 Unit test: denial short-circuit — fake repository call
       count of zero

## 6. Integration + housekeeping
- [x] 6.1 Integration smoke (emulator, repository-seamed fake
       pipeline): submit → job → preview → apply → summary;
       deterministic (no network LLM in CI)
- [x] 6.2 Entry point: seasons toolbar "AI import" action
- [x] 6.3 SPDX headers; format/analyze/breakdown_lints clean;
       coverage + coverde gate; gitleaks (confirms no secret material
       — masked input, stored id only)
- [x] 6.4 `openspec` coverage audit: every scenario in
