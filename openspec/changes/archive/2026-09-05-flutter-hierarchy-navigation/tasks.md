...
- [x] 1.1 `lib/domain/reconciliation/` — move
...
- [x] 1.2 Migrate `features/seasons/seasons_controller.dart` onto the
       shared module; seasons golden tests must remain byte-stable
       (parity proof)
- [x] 1.3 Unit tests: runner with fake scheduler — attempt budget,
...
- [x] 2.3 `data/episode_repository.dart` — `listBySeason(seasonId)`
       ...
       `create(request)`
- [x] 2.4 `data/scene_repository.dart` — `listByEpisode(episodeId)`,
       `create(request)`
- [x] 2.5 `data/costume_category_repository.dart` — `list(seasonId)`,
       `create`, `rename(id, version, name)`, `archive(id)`
- [x] 2.6 Unit tests for every repository method: Ok AND Err branches;
       cache untouched on fetch failure; snapshot-replace removes deleted
       rows

## 3. Membership read
- [x] 3.1 `lib/auth/season_membership_provider.dart` — family provider,
       strict `SeasonMembershipDto` capability parsing (unknown string →
       `Err`), TTL-scoped caching
- [x] 3.2 Unit tests: strict parser (known set / unknown entry),
       provider refetch-vs-TTL behavior with fake clock

## 4. Controllers
- [x] 4.1 `features/blocks/` — family `BlocksController(seasonId)` on
...
- [x] 4.2 `features/episodes/` — `EpisodesController(blockId, seasonId)`
       (server-side `?block_id=` filter, `groupByBlock` for merged renders),
       create gated likewise
- [x] 4.3 `features/scenes/` — `ScenesController(episodeId)`, create
       gated likewise
- [x] 4.4 `features/costume_categories/` — controller + next-order-key
...
- [x] 6.1 Per screen: data / empty / error / stale / overlay-state
       widget tests (semantic finders, paired with goldens)
- [x] 6.2 Goldens {light, dark} × {android, macos} for every non-trivial
       surface (4 variants each)
- [x] 6.3 404-narrative + pop-back tests; membership chip states; strict-
       parse error state
- [x] 6.4 Categories: toggle, rename-409 copy, archive flow, order-key
       edge derivation surfaced in list order

## 7. Integration tests (Tier 4)
- [x] 7.1 On-emulator smoke (dev-auth backend): create season → block →
       episode → scene → category; each appears after projector lag
       within the reconciliation budget; back navigation preserves state
- [x] 7.2 Sign-out mid-navigation returns to the login gate (depends on
       `flutter-login-and-app-shell`)

## 8. Housekeeping
- [x] 8.1 SPDX headers; `dart format` / `flutter analyze` /
       `breakdown_lints_runner` clean; `flutter test --coverage` +
       `coverde` changed-code gate; no new packages (pure Flutter usage)
- [x] 8.2 `openspec` coverage audit: every scenario in
       `flutter-hierarchy-navigation` and
       `flutter-costume-categories-screen` has a passing test
- [x] 8.3 Backend issue #335 landed (PR #355): the episodes `block_id`
