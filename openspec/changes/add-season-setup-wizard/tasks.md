...
- [ ] 1.1 Create the screen spec `docs/design/screens/
      ...
      diagram)
- [ ] 1.2 Glossary: add the copy keys for wizard texts (step labels,
...
- [ ] 2.1 `lib/features/seasons/setup/setup_wizard_state.dart`:
      ...
      0)
- [ ] 2.2 `setup_wizard_controller.dart`: `@riverpod` controller;
...
- [ ] 2.3 Smart-default season number (max+1 from the seasons state)
      as a pure function + unit tests (empty list, collision, gaps)

## 3. Dispatch Sequence (Unit/Widget Tier)

- [ ] 3.1 Dispatch reduction over the existing repositories
...
- [ ] 3.2 Progress state per command (`dispatching` phase with
...
- [ ] 3.3 Abort semantics: `PopScope` + confirmation dialog in
      editing/review; disabled during dispatch — widget tests for
      both paths

## 4. Step Widgets (Widget Tier)

- [ ] 4.1 Season step: number field (smart default), optional name,
      live "Season n" preview — widget tests including validation
      copy per error code
- [ ] 4.2 Blocks step: repeatable block drafts (episode count,
...
- [ ] 4.3 Review step: summary (season + blocks + total episodes),
      confirm CTA — goldens
- [ ] 4.4 Completion screen: result overview; conditional CTA
      ...
      states
- [ ] 4.5 Progress indicator "Schritt x von n" + directional step
      transitions (semantic labels, correct dark/light)

## 5. Entry Points & Integration

- [ ] 5.1 Wizard page pushed on the Season tab's navigator; entry
...
- [ ] 5.2 Gherkin `features-spec/setup/season-wizard.feature`: happy
      path, template application, partial failure + retry, abort
      with discard (runs on device via flutter_gherkin)
- [ ] 5.3 Integration test on an emulator: full wizard run against
      the dev backend (season → 2 blocks → review → completion)
      including the abort path

## 6. Tests, Goldens & Wrap-up

- [ ] 6.1 Goldens: all 4 steps + completion (light/dark), validation
      and error states
- [ ] 6.2 `flutter analyze` + `breakdown_lints`, format, coverde
      threshold on changed code green
- [ ] 6.3 `openspec validate add-season-setup-wizard` — change
